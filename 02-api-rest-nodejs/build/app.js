...
async function transactionsRoutes(app2) {
  ...
  app2.get("/summary", {
    ...
    const { sessionId } = request.cookies;
    const summary = await knex("transactions").where("session_id", sessionId).sum("ammount", { as: "amount" }).first();
    return { summary };
  });
  app2.post("/", async (request, reply) => {
    const getTransactionBodySchema = import_zod2.z.object({
      title: import_zod2.z.string(),
      amount: import_zod2.z.number(),
      type: import_zod2.z.enum(["credit", "debit"])
    });
    const { title, amount, type } = getTransactionBodySchema.parse(request.body);
    ...
    await knex("transactions").insert({
      ...
      title,
      ammount: type === "credit" ? amount : amount * -1,
      session_id: sessionId
    });
